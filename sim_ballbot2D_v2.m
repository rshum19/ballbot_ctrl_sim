function sim_ballbot2D_v2(X0, sim_params, sim_opts)
    close all;
...
function [dx u x_d] = odefun_dynamicsBB(t, x, u, sim_params)
...
function [t_out_hist,X_out_hist] = simulate()
    
    
    ode_opts = odeset('Events',@bbfallEvent2) ;
    
    Ti = sim_params.t_range(1);
    Tf = sim_params.t_range(end);
    dt = sim_params.dt;
    
    % Initialize variables 
    err = 0;        % Error
    err_dot = 0;    % First derivative of error
    err_ddot = 0;   % Seocnd derivative of error
    ...
    for t = sim_params.t_range
        ...
        err = 
        err_ddot = err_dot + (phi - phie);
        
        sim_params.ctrl_params.err = err;
        sim_params.ctrl_params.err_dot = err_dot;
        sim_params.ctrl_params.err_ddot = err_ddot;
        
        % Call controller
        [u,uparts] = sim_params.ctrl(t,x,x_d,sim_params.ctrl_params);
    
        % Integration step 
        [t_out X_out] = ode45(@(t,x)odefun_dynamicsBB(t,x,u,sim_params), [t t+dt], X0,ode_opts) ;
       
        % Initial conditions for differential equaitons
        X0 = X_out(end,:);
        
        % Record history of states and inputs
        X_hist(i,:) = X_out(end,:);
        u_hist(i) = u;
        t_hist(i) = t;
