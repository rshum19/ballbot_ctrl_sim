function sim_ballbot3D(X0, sim_params, sim_opts)
    close all;
    ...
    u_out = zeros(length(t_out),2);
    %X_d_out = zeros(1,size(X0,1));
    X_d_out = zeros(1,4);
...
%         animate(t_out,X_out,Anim);
        ...
        X_anim = [X_out(:,1),X_out(:,3),X_out(:,2),X_out(:,4),X_out(:,5),X_out(:,7),X_out(:,6),X_out(:,8)];

        figure(1);
        h = sim_opts.animate_fun(X_anim(1,:));
        axis([-2 2 -2 2 0 2]);
        grid on
        ...
        fprintf(1,'t:\n');
        while t < t_out(end)
            idx = find(t_out>t,1,'first');
            sim_opts.animate_fun(X_anim(idx,:), h);
            drawnow;
            fprintf(1,'\b\b\b\b\b\b%0.3f',t);
            t = toc;
        end
    end
...
function [dx u x_d] = odefun_dynamicsBB(t, x, sim_params)
    ...
    % Call time-deriviate of state (dx)
...
value = [abs(x(3)) - pi/2, abs(x(4)) - pi/2];
isterminal  = [1,1];
direction = [1,1];
